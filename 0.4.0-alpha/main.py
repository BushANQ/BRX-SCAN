...
╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝    ╚══════╝ ╚═════╝╚═╝  ╚═╝ ╚═════╝
    ...
    def gather_information(self, ip):
        """收集目标信息"""
        print("\n[*] 开始信息收集...")
        ...
        try:
            ...
            # 子域名枚举
            subdomains = self.enumerate_subdomains(ip)
            if subdomains:
                print("[+] 发现子域名:")
            ...
            # SSL证书信息
            ssl_info = self.get_ssl_info(ip)
            if ssl_info:
                print("[+] SSL证书信息:")
                print(ssl_info)
            
            # CORS配置检查
            cors_info = self.check_cors(ip)
            if cors_info:
                print("[+] CORS配置:")
                print(cors_info)
            
            # HTTP头信息
            headers_info = self.get_http_headers(ip)
            if headers_info:
                print("[+] HTTP响应头:")
                print(headers_info)
    ...
    def get_whois_info(self, ip):
        ...
        try:
            ...
            # 匹配模式
            patterns = {
                "域名": r"Domain Name:\s*([\w.-]+)",
                "注册ID": r"ROID:\s*([\w-]+)",
                "域名状态": r"Domain Status:\s*([\w\s]+)",
                "注册人": r"Registrant:\s*(.+)",
                "注册邮箱": r"Registrant Contact Email:\s*(.+)",
                "注册商": r"Sponsoring Registrar:\s*(.+)",
                "域名服务器": r"Name Server:\s*([\w.-]+)",
                "创建时间": r"Registration Time:\s*(.+)",
                "过期时间": r"Expiration Time:\s*(.+)",
                "DNSSEC": r"DNSSEC:\s*(.+)"
            }
            
            for key, pattern in patterns.items():
                matches = re.findall(pattern, whois_text, re.MULTILINE)
                if matches:
                    if key == "域名服务器":
                        result[key] = matches  # 保存所有匹配的域名服务器
                    else:
    ...
    def get_dns_info(self, ip):
        ...
        try:
            ...
            records = ['A', 'AAAA', 'MX', 'NS', 'TXT', 'SOA', 'CNAME', 'PTR']
            results = {}
    ...
    def enumerate_subdomains(self, ip):
        """子域名枚举"""
        try:
            domain = socket.gethostbyaddr(ip)[0]
            subdomains = set()
            ...
            common_prefixes = [
                ...
                'ns1', 'ns2', 'smtp', 'secure', 'vpn', 'api', 'm', 'dev',
                'portal', 'admin', 'test', 'stage', 'git', 'imap', 'pop',
                'ftp', 'db', 'monitor', 'backup', 'cdn', 'cloud', 'wiki'
            ]
    ...
    def get_ssl_info(self, ip):
        ...
        try:
            ...
            with context.wrap_socket(socket.socket(), server_hostname=ip) as sock:
                sock.connect((ip, 443))
                cert = sock.getpeercert(True)
                x509 = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_ASN1, cert)
                
                result = {
                    "主题": dict(x509.get_subject().get_components()),
                    "颁发者": dict(x509.get_issuer().get_components()),
                    "版本": x509.get_version(),
                    "序列号": x509.get_serial_number(),
                    "有效期从": x509.get_notBefore().decode(),
                    "有效期至": x509.get_notAfter().decode(),
                    "签名算法": x509.get_signature_algorithm().decode()
                }
                return json.dumps(result, indent=2, ensure_ascii=False)
        except Exception as e:
            if self.options.verbose:
            ...
            for url in urls:
                try:
                    response = requests.get(url, headers={'User-Agent': random.choice(self.user_agents)},
                                         verify=False, timeout=5)
                    results[url] = dict(response.headers)
                except:
                    continue
...
def main():
    ...
    parser.add_argument('--info-gathering', action='store_true', help='执行信息收集', default=True)  # 添加信息收集开关,默认开启
    ...
    try:
        ...
        for host in hosts:
            ...
            # 强制执行信息收集
            if args.info_gathering:
                print("[*] 开始执行信息收集...")
                scanner.gather_information(host)
                print("[*] 信息收集完成\n")
